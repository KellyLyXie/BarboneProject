...
export async function extractTickerFromQuery(query: string, region: string, lang: string): Promise<string> {
  ...
  Language: "${lang}"
    ...
    // 从环境变量中读取 GPT-4o mini API 的 URL 和 Key
    const apiEndpoint = process.env.NEXT_PUBLIC_GPT4O_MINI_API_URL || "";
    const apiKey = process.env.NEXT_PUBLIC_GPT4O_MINI_API_KEY || "";
  
    // 如果未配置，抛出错误
    if (!apiEndpoint) {
      throw new Error("GPT-4o mini API endpoint is missing. Please set NEXT_PUBLIC_GPT4O_MINI_API_URL in .env.local");
    }
    if (!apiKey) {
      throw new Error("GPT-4o mini API key is missing. Please set NEXT_PUBLIC_GPT4O_MINI_API_KEY in .env.local");
    }
  
    // 构造请求体。注意，根据你使用的 GPT-4o mini API 文档，可能与 OpenAI API 格式相似
    const requestBody = {
      model: "gpt-3.5-turbo", // 如果你用 GPT-4o mini，可能需要确定正确的 model 名称
      messages: [
    ...
    // 假设返回格式类似于 OpenAI 的： { choices: [ { message: { content: "AAPL" } } ] }
